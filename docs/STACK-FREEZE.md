...
|---|---|---|
| Newly created project (during submission period) | This repository, created 2025-08-15 | ✅ Met |
...
Seven tables. No SQL in this document — SQL is milestone 0D.
...
```
PURPOSE:      Optional dedicated table for pre-computed embeddings when
              content and vector index benefit from separation.
IDENTITY:     vector_id (UUID)
CRITICAL:     event_id (FK), model_name, embedding (VECTOR), indexed_at
AUTHORITY:    None.
RETENTION:    Re-computable — can be rebuilt from memory_events.
VECTOR:       YES — primary vector index lives here if separated from events
```

*Note: if embedding joins are simple enough, memory_vectors may be merged into
memory_events in milestone 0D. Decision deferred to schema design.*
